...
def load_and_process_data(reports_dir):
    """Load the CSV data and process it for graphing"""
       csv_path = Path(f'../{reports_dir}/area_timing_summary.csv')
       if not csv_path.is_file():
        print(f"\nERROR: Expected FloPoCo data at: {csv_path}")
        print("Expected format: CSV with columns including 'Operator', 'Bitwidth', 'latency_cycles', 'Operating_Clock_Period_ns', and resource usage.")
        exit(1)
...
def load_vivado_reference_data():
    ...
    try:
        csv_path = Path('../reports_vivado/reference_vivado_implementations.csv')
        if not csv_path.is_file():
        print(f"\nERROR: Expected Vivado reference data at: {csv_path}")
        print("Expected format: CSV with columns including 'Operator', 'Input_Type', 'Max_Frequency_MHz', and resource usage.")
        return None
...
def load_vivado_reference_data():
    """Load Vivado reference implementation data (same format as FloPoCo)"""
    csv_path = Path('../reports_vivado/area_timing_summary.csv')
    if not csv_path.is_file():
        print(f"\nERROR: Expected Vivado reference data at: {csv_path}")
        print("Expected format: CSV with columns including 'Operator', 'Bitwidth', 'latency_cycles', 'Operating_Clock_Period_ns', and resource usage.")
        return None

    vivado_df = pd.read_csv(csv_path)
    vivado_df['Op_Bitwidth'] = vivado_df['Operator'] + '_' + vivado_df['Bitwidth'].astype(str)
    vivado_df['Source'] = 'Vivado'
    ...
    for resource in resource_columns:
        ...
        for idx, op_bitwidth in enumerate(sorted(common_pairs)):
            ...
            if len(flopoco_subset) > 0:
                ...
                # Add latency annotations for FloPoCo points
                for _, row in flopoco_subset.iterrows():
                    freq = 1000 / row['Operating_Clock_Period_ns']
                    ax.annotate(f'{row["latency_cycles"]:.0f}cyc', 
                               (row[resource], freq),
                               xytext=(5, 5), textcoords='offset points', 
            ...
            if len(vivado_subset) > 0:
                ...
                # Add latency labels for Vivado points
                for _, row in vivado_subset.iterrows():
                    freq = 1000 / row['Operating_Clock_Period_ns']
                    # Check if Vivado data has latency_cycles column, otherwise use a default or skip
                    if 'latency_cycles' in row and pd.notna(row['latency_cycles']):
                        latency_text = f'Vivado\n{row["latency_cycles"]:.0f}cyc'
                    else:
                        latency_text = 'Vivado\n(latency N/A)'
                    
                    ax.annotate(latency_text, 
                               (row[resource], freq),
